...
def summarize_directory(dir_name: str):
    ...
    # Find all CSV files (excluding test_config and existing summary files)
    ...
    for csv_file in sorted(csv_files):
        ...
        try:
            with open(csv_file, 'r', newline='') as f:
                ...
                for row in reader:
                    ...
                    try:
                        ...
                        print(f"⚠️  Warning: Could not parse num_trials in {csv_file.name}, row: {row}")
                        continue
                    ...
                    try:
                        ...
                        print(f"⚠️  Warning: Could not parse total_time in {csv_file.name}, row: {row}")
                        continue
            ...
            if num_trials_list and total_time_list:
                ...
                print(f"✓ {file_name:<30} branches={len(num_trials_list):<3} avg_trials={avg_num_trials:>8.2f} avg_time={avg_total_time:>8.3f}s")
            else:
                print(f"⚠️  {file_name:<30} (no data)")
