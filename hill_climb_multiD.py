...
class MetadataCompressionOriginalSpace:
    """Keep compression metadata in the original X-space."""
...
def detect_compression_basin(fitness_func, local_min_x, max_search=100, verbose = False):
    """
    Bidirectional basin detection.

    Rules:
    1) current_fit == local_fit: keep extending (track farthest equal-fitness point)
    2) current_fit > local_fit: keep scanning (still inside basin/shoulder)
    3) current_fit < local_fit: treat as exit boundary and stop

    If an exit is found (Rule 3), use it. Otherwise, fall back to the farthest
    equal-fitness point from Rule 1.


    Returns:
        (start_x, length) or None
    """
    def debug_print(msg):
        if verbose:
            print(msg)
...
def hill_climb_with_compression_nd_code(
    ...
    for it in range(max_iterations):
        ...
        # Restart: try basin boundaries (start-1, end+1)
        restart_candidates = []
