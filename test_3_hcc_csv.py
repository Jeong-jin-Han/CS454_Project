...
# ===============================
# Diagonal Basin Detection (No Compression)
# ===============================
def detect_diagonal_basin(fitness_fn, point, dim_pair, max_search=100, verbose=False):
    """
    Detect basin along DIAGONAL direction when no axis-aligned basin found.
    
    This finds escape routes along diagonal directions (e.g., X+Y, X-Y in 2D).
    Returns boundary points for restart, but does NOT create compression.
    ...
    point : tuple
        Current stuck point
    dim_pair : tuple (d1, d2, direction)
        (dim1, dim2, +1 or -1)
        +1: both increase (++) or both decrease (--)
        -1: d1 increases, d2 decreases (+-)
    max_search : int
        Maximum search distance
    
    Returns:
    --------
    List of (boundary_point, fitness) or None
    """
    d1, d2, direction = dim_pair
    local_fitness = fitness_fn(point)
    
    if verbose:
        dir_str = f"dim{d1}+dim{d2}" if direction == 1 else f"dim{d1}-dim{d2}"
        print(f"    🔍 Diagonal search [{dir_str}] from {point}: ", end="")
    
    boundary_points = []
    
    # Search POSITIVE diagonal (+step in d1, +direction*step in d2)
    for step in range(1, max_search + 1):
        new_pt = list(point)
        new_pt[d1] = point[d1] + step
        new_pt[d2] = point[d2] + direction * step
        new_pt_tuple = tuple(new_pt)
        
        new_fitness = fitness_fn(new_pt_tuple)
        
        # Rule 1: Same fitness (plateau continues)
        if abs(new_fitness - local_fitness) < 1e-9:
            continue
        # Rule 3: Better fitness (exit!) - This is our escape route!
        elif new_fitness < local_fitness:
            boundary_points.append((new_pt_tuple, new_fitness))
            if verbose:
                print(f"EXIT ✅ {new_pt_tuple}, f={new_fitness:.4f}")
            break
        # Rule 2: Worse fitness (can't escape this way)
        else:
            break
    
    # Search NEGATIVE diagonal (-step in d1, -direction*step in d2)
    if not boundary_points:  # Only search if positive didn't find exit
        for step in range(1, max_search + 1):
            new_pt = list(point)
            new_pt[d1] = point[d1] - step
            new_pt[d2] = point[d2] - direction * step
    ...
    if not boundary_points and verbose:
        print(f"no exit ❌")
...
def hill_climb_with_compression_nd_code(
    ...
    for it in range(max_iterations):
        ...
        # -----------------------------------------------------------
        if verbose:
            print(f"\n⚠️ Stuck at {point}, f={f:.6g}, detecting basins…")
        ...
        for d in active_dims:
            ...
            if basin:
                ...
                basins[d] = basin
                if verbose:
                    print(f"  ✅ Axis-aligned basin detected on dim {d}: {basin}")
        
        if verbose and not basins:
            print(f"  ❌ No axis-aligned basins detected at {point}, f={f:.6g}")

        if not basins:
            # ============================================================
            # STUCK: Try UPHILL MOVEMENT to escape
            # ============================================================
            if verbose:
                print(f"\n  ⚠️ STUCK (no basins detected)")
                print(f"     Position: {point}, f={f:.6g}")
                print(f"     Trying UPHILL MOVEMENT to escape pit...")
            
            # Generate escape candidates with small jumps
            escape_candidates = []
            jump_distances = [3, 5, 8, 10]  # Small jumps to explore nearby
            
            if dim == 1:
                for dist in jump_distances:
                    for direction in [-1, 1]:
                        esc_pt = (point[0] + direction * dist,)
                        escape_candidates.append(esc_pt)
            
            elif dim == 2:
                # Try 8 directions at multiple distances
                for dist in jump_distances:
                    for dx in [-1, 0, 1]:
                        for dy in [-1, 0, 1]:
                            if dx == 0 and dy == 0:
                                continue
                            esc_pt = (point[0] + dx * dist, point[1] + dy * dist)
                            escape_candidates.append(esc_pt)
            
            else:
                # N-D: try random directions
                for _ in range(20):
                    dist = random.choice(jump_distances)
                    esc_pt = tuple(
                        point[i] + random.choice([-1, 1]) * dist 
                        for i in range(dim)
                    )
                    escape_candidates.append(esc_pt)
            
            # Evaluate escape options
            escape_options = []
            for esc_pt in escape_candidates:
                if time_limit and (time.time() - start_time) >= time_limit:
                    return traj, cm
                esc_f = fitness_func_nd_code(esc_pt)
                escape_options.append((esc_pt, esc_f))
            
            if escape_options:
                # Pick BEST escape (accept even if uphill!)
                best_esc_pt, best_esc_f = min(escape_options, key=lambda x: x[1])
                
                is_uphill = best_esc_f > f
                
                if verbose:
                    print(f"\n  🎯 ESCAPE VIA UPHILL JUMP:")
                    print(f"     From: {point}, f={f:.6g}")
                    print(f"     To: {best_esc_pt}, f={best_esc_f:.6g}")
                    if is_uphill:
                        print(f"     Status: ⚠️ UPHILL (accepting worse fitness!)")
                        print(f"     Delta: +{best_esc_f - f:.6g}")
                    else:
                        print(f"     Status: ✅ Downhill")
                    print(f"  ➡️ Continuing search from new position...\n")
                
                point, f = best_esc_pt, best_esc_f
                traj.append((point, f, False))
                
                # Check if escape hit goal
                if abs(f) < global_min_threshold:
                    if verbose:
                        print("  🎉 Escape hit goal!")
                    break
                
                # Continue from escaped position
                continue
            
            else:
                # Could not generate escapes
                if verbose:
                    print(f"  ❌ Could not escape (time limit?)")
                break
        ...
        if basins:
            ...
            if verbose:
                print(f"\n  ✅ AXIS-ALIGNED RESTART")
                print(f"     From: {point}, f={f:.6g}")
                print(f"     To: {restart_point}, f={restart_f:.6g}")
                print(f"     Basins used: {len(basins)} dimension(s)")
                print(f"  ➡️ Continuing climb with compression...\n")
