    ...
    for it in range(max_iterations):
        ...
        while step_count < max_steps_per_iteration:
            ...
            for d in active_dims:
                ...
                if comp_sys:
                    z = comp_sys.forward(point[d])
                    n_vals = [comp_sys.inverse(z - 1), comp_sys.inverse(z + 1)]
                else:
                    n_vals = [point[d] - 1, point[d] + 1]
                
                for val in n_vals:
                    cand_pt = list(point)
                    cand_pt[d] = val
                    cand_pt_t = tuple(cand_pt)
                ...
                for d1, d2 in itertools.combinations(active_dims, 2):
                    ...
                    for v1 in n1_vals:
                        for v2 in n2_vals:
                            ...
                            diag_point[d2] = v2
                            cand = tuple(diag_point)
                            candidates.append((cand, fitness_func_nd_code(cand), [d1, d2]))  # (point, modified_dims)

            # Pick best neighbor (steepest descent)
            best_point, best_f = point, f
            meaningful_dims = set()
            for cand_point, cand_f, modified_dims in candidates:
                if cand_f < best_f:
                    best_point, best_f = cand_point, cand_f
                if cand_f != f:
                    for d in modified_dims:
                        meaningful_dims.add(d)
            
            for d in range(dim):
                if d not in meaningful_dims:
                    dim_stagnation[d] += 1  # Increment stagnation counter
                else:
                    dim_stagnation[d] = 0  # Reset stagnation counter for meaningful dimensions

            # Deactivate dimensions that have been stagnant for too long
            for d in list(active_dims):
                if dim_stagnation[d] >= deactivation_patience:
                    active_dims.remove(d)
                    print(f"Deactivating dimension {d} due to stagnation.")
